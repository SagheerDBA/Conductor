"""
Conductor -- gmail_tool.py

Provides two Conductor-level tools for Gmail access:
  search_gmail       -- search messages by query, return metadata list
  read_email_thread  -- read full thread content (all messages, plain text)

Reuses OAuth credentials from the local Gmail MCP server files.
Supports two accounts:
  personal  -- sagheeremail@gmail.com       (HOME/.gmail-mcp/credentials.json)
  work      -- sagheeremail.wrk@gmail.com   (HOME/gmail-work-home/.gmail-mcp/credentials.json)
...
# ---------------------------------------------------------------------------
# Credential paths -- absolute so the tool works from any working directory
# ---------------------------------------------------------------------------
...
# Truncate body text at this length to keep tool results manageable
MAX_BODY_CHARS = 4000
...
def _gmail_get(account: str, path: str, params: dict = None) -> dict:
    """GET request to Gmail API. Auto-refreshes token on 401."""
...
def search_gmail(account: str, query: str, max_results: int = 10) -> dict:
    ...
    Returns a list of {message_id, thread_id, subject, from, date} dicts.
...
def read_email_thread(account: str, thread_id: str) -> dict:
    ...
    Returns all messages with from/date/subject/body (plain text, truncated at 4000 chars).
